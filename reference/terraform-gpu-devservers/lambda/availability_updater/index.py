...
def update_gpu_availability(gpu_type: str, k8s_client=None) -> None:
    ...
    try:
        ...
        matching_asgs = [
            asg
            for asg in all_asgs_response["AutoScalingGroups"]
        ...
        if k8s_client is not None:
            try:
                logger.info(
                    f"Starting Kubernetes query for {gpu_type} GPU availability"
                )
            ...
            logger.warning(
                f"No Kubernetes client available for {gpu_type}, using ASG-based calculation"
            )
            # Fallback to ASG-based calculation (assume all GPUs available)
            available_gpus = total_gpus
...
def check_schedulable_gpus_for_type(k8s_client, gpu_type: str) -> int:
    ...
    try:
        ...
        logger.info(
            f"Retrieved {len(nodes.items) if nodes.items else 0} nodes for {gpu_type}"
        )
        ...
        for i, node in enumerate(nodes.items):
            logger.info(
                f"Processing node {i + 1}/{len(nodes.items)}: {node.metadata.name}"
            )

            if not is_node_ready_and_schedulable(node):
                logger.info(
                    f"Node {node.metadata.name} is not ready/schedulable, skipping"
                )
                continue

            logger.info(
                f"Node {node.metadata.name} is ready, checking GPU availability"
            )
            # Get available GPUs on this node
            available_on_node = get_available_gpus_on_node(v1, node)
            total_schedulable += available_on_node
            logger.info(
                f"Node {node.metadata.name}: {available_on_node} GPUs available"
            )
