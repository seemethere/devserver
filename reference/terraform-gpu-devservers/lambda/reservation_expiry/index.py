...
def handler(event, context):
    ...
    try:
        ...
        if (
            event.get("source") == "cloudwatch.schedule"
            and event.get("cleanup_type") == "snapshots"
        ):
        ...
        try:
            ...
            logger.info(
                f"Found {len(expired_cancelled_reservations)} expired/cancelled reservations"
            )
            ...
            for reservation in expired_cancelled_reservations:
                ...
                expired_at = reservation.get(
                    "expired_at", reservation.get("cancelled_at", "")
                )
                if not expired_at:
    ...
    try:
        ...
        if pod_name:
            ...
            try:
                cleanup_pod(
                    pod_name,
                    reservation.get("namespace", "gpu-dev"),
...
def cleanup_pod(
    pod_name: str, namespace: str = "gpu-dev", reservation_data: dict = None
) -> None:
    ...
    try:
        ...
        try:
            ...
            if user_id and volume_id:
                logger.info(
                    f"Found persistent storage in reservation data: volume {volume_id} for user {user_id}"
                )
            ...
            if user_id and volume_id:
                logger.info(
                    f"Creating shutdown snapshot for user {user_id}, volume {volume_id}"
                )
                snapshot_id = create_pod_shutdown_snapshot(volume_id, user_id)
                if snapshot_id:
                    logger.info(
                        f"Shutdown snapshot {snapshot_id} initiated for {pod_name}"
                    )
                else:
                    logger.warning(f"Failed to create shutdown snapshot for {pod_name}")
            else:
                logger.info(
                    f"No persistent storage found for pod {pod_name} - skipping shutdown snapshot"
                )

        except Exception as snapshot_error:
            logger.warning(
                f"Error creating shutdown snapshot for {pod_name}: {snapshot_error}"
            )
