...
EOF
        ...
        env=[
            client.V1EnvVar(
                name="AWS_REGION", value=os.environ.get("REGION", "us-east-2")
            ),
        ],
...
def parse_buildkit_progress(logs: str) -> str:
    ...
    for line in reversed(recent_lines):
        download_match = re.search(
            r"sha256:\w+.*?(\d+\.?\d*\w+)\s*/\s*(\d+\.?\d*\w+)", line
        )
...
def wait_for_buildkit_job(
    k8s_client, job_name: str, timeout_seconds: int = 600, progress_callback=None
) -> Dict[str, Any]:
    """
...
def _get_job_logs(core_v1, job_name: str) -> str:
    ...
    try:
        ...
        pod_list = core_v1.list_namespaced_pod(
            namespace="gpu-dev", label_selector=f"job-name={job_name}"
        )
