...
EFS_SUBNET_IDS = (
    os.environ.get("EFS_SUBNET_IDS", "").split(",")
    if os.environ.get("EFS_SUBNET_IDS")
    else []
)
...
def needs_ebs_migration(user_id, target_az):
    ...
    try:
        ...
        if not volumes:
            logger.info(
                f"No existing EBS volumes found for user {user_id} - no migration needed"
            )
        ...
        logger.info(
            f"User {user_id} needs EBS migration: volume {current_volume_id} from {current_az} to {target_az}"
        )
...
def migrate_ebs_across_az(user_id, current_volume_id, current_az, target_az):
    ...
    try:
        ...
        waiter.wait(
            SnapshotIds=[snapshot_id], WaiterConfig={"Delay": 15, "MaxAttempts": 240}
        )  # Up to 1 hour
        ...
        waiter.wait(
            VolumeIds=[new_volume_id], WaiterConfig={"Delay": 5, "MaxAttempts": 60}
        )
...
def update_reservation_error(
    reservation_id: str, error_message: str, error_field: str = "failure_reason"
) -> None:
...
def find_reservation_by_prefix(reservation_id: str, user_id: str = None) -> dict:
    ...
    try:
        ...
        if (
            len(reservation_id) == 36 and reservation_id.count("-") == 4
        ):  # Full UUID format
            try:
...
def query_user_reservations_with_prefix(
    table, user_id: str, reservation_prefix: str
) -> list:
            ...
            try:
                ...
                initial_record = {
                    ...
                    "name": reservation_request.get(
                        "name", f"Multinode {node_index + 1}/{total_nodes}"
                    ),
...
def check_all_multinode_nodes_ready(
    master_reservation_id: str, total_nodes: int
) -> bool:
...
def coordinate_multinode_reservation(
    master_reservation_id: str, total_nodes: int
) -> bool:
        ...
        if available_gpus >= total_gpus_needed:
            ...
            if success_count == total_nodes:
                ...
                fail_all_multinode_reservations(
                    ...
                    f"Partial processing failure ({success_count}/{total_nodes})",
                )
                return False
        else:
...
def process_multinode_individual_node(message_body: dict) -> bool:
    ...
    try:
        ...
        if (
            "reservation_id" in locals()
            and "node_index" in locals()
            and "total_nodes" in locals()
        ):
            update_multinode_pod_status(
...
def update_all_multinode_status(
    master_reservation_id: str, status: str, failure_reason: str = None
):
...
def update_multinode_pod_status(
    reservation_id: str,
    pod_status: str,
    node_index: int = None,
...
def queue_all_multinode_reservations(
    master_reservation_id: str,
...
def calculate_multinode_queue_position_and_wait_time(
    master_reservation_id: str,
    total_gpus_needed: int,
    gpu_type: str,
    available_gpus: int,
    ...
    try:
        ...
        queued_reservations = scan_dynamodb_paginated(
            ...
            ExpressionAttributeNames={"#status": "status"},
            ExpressionAttributeValues={":status": "queued", ":gpu_type": gpu_type},
        )
        ...
        if total_gpus_ahead > 0:
            ...
            estimated_wait_minutes = int(
                (total_gpus_ahead / max(available_gpus, 1))
                    ...
                    if expires_at:
                        try:
                            ...
                            if minutes_until_expiry > 0:
                                if (
                                    earliest_expiry_minutes is None
                                    or minutes_until_expiry < earliest_expiry_minutes
                                ):
        ...
        if reservation_id:
            try:
                ...
                if reservation_request.get("dockerfile_s3_key"):
                    initial_record["dockerfile_base64_data"] = reservation_request[
                        "dockerfile_s3_key"
                    ]
...
def allocate_gpu_resources(reservation_id: str, request: dict[str, Any]) -> None:
    ...
    try:
        ...
        github_user = request.get("github_user", user_id)
        ...
        if use_persistent_disk:
            try:
                ...
                update_reservation_status(
                    reservation_id,
                    "preparing",
                    detailed_status="Reserving persistent disk slot",
                )
                logger.info(
            ...
            try:
                ...
                if migration_needed:
                    ...
                    latest_snapshot = get_latest_snapshot(
                        user_id, current_volume_id, include_pending=True
                    )
                    ...
                    persistent_volume_id, is_new_disk = (
                        create_or_find_persistent_disk_in_az(user_id, target_az)
                    )
                    logger.info(
        ...
        disk_status = (
            "with persistent disk" if use_persistent_disk else "without persistent disk"
        )
        ...
        logger.info(
            f"MAIN FLOW: SSH connectivity test result for {reservation_id}: ssh_ready={ssh_ready}"
        )
    ...
    try:
        ...
        try:
            ...
            reservations_table.update_item(
                ...
                ExpressionAttributeValues={":history": [new_entry]},
            )
...
def create_kubernetes_resources(
    ...
    try:
        ...
        if (
            "monitor_stop_event" not in locals()
            and reservation_id not in _monitoring_threads
        ):
...
def get_pod_resource_limits(
    gpu_count: int, gpu_type: str, is_multinode: bool = False
) -> dict:
    ...
    use_efa = (
        gpu_type != "t4-small"  # EFA not supported on t4-small
        and is_multinode  # Only for multinode deployments
        and gpu_count == max_gpus  # Only when using all GPUs per node
    )
...
EOF
                    ...
                    env=[
                        ...
                        client.V1EnvVar(
                            name="CREATE_SH_ENV",
                            value=str(is_new_disk or recreate_env).lower(),
                        ),
                        client.V1EnvVar(
                            name="USE_PERSISTENT_DISK",
                            value=str(use_persistent_disk).lower(),
                        ),
                        client.V1EnvVar(name="GPU_TYPE", value=gpu_type.upper()),
                        client.V1EnvVar(
                            name="SUPPORTS_EFA",
                            value=str(
                                _pod_uses_efa(gpu_count, gpu_type, is_multinode)
                            ).lower(),
                        ),
                    ]
                    ...
                    + (
                        [
                            client.V1VolumeMount(
                                name="shared-efs", mount_path="/shared-personal"
                            )
                        ]
...
def create_or_find_persistent_disk_in_az(
    user_id: str, availability_zone: str
) -> tuple[str, bool]:
    ...
    try:
        ...
        if volumes:
            ...
            logger.info(
                f"Found existing persistent disk {volume_id} for user {user_id} in {availability_zone}"
            )
...
def create_or_find_persistent_disk(user_id: str) -> tuple[str, bool]:
    ...
    try:
        ...
        if volumes:
            volume_id = volumes[0]["VolumeId"]
            logger.info(
                f"Found existing persistent disk {volume_id} for user {user_id}"
            )
            return volume_id, False  # existing disk
...
def should_use_persistent_disk(user_id: str, current_reservation_id: str) -> bool:
    ...
    try:
        ...
        reservations_with_persistent_disk = [
            ...
            or res.get("ebs_volume_reserved") == True
        ]
...
def update_reservation_connection_info(
    ...
    try:
        ...
        if persistent_volume_id:
            ...
            update_fields["ebs_volume_reserved"] = (
                False  # Clear reservation flag once volume is attached
            )
...
def start_background_pod_monitoring(
    k8s_client, pod_name: str, reservation_id: str
) -> threading.Event:
...
def update_pod_status_and_events(
    k8s_client, pod_name: str, reservation_id: str
) -> dict:
    """
    ...
    try:
        ...
        try:
            ...
            if e.status == 404:
                ...
                update_reservation_status(
                    reservation_id,
                    "preparing",
                    detailed_status="⏳ Pod creation pending",
                )
                return {
                    "phase": "Pending",
        ...
        events = v1.list_namespaced_event(
            namespace="gpu-dev", field_selector=f"involvedObject.name={pod_name}"
        )
        ...
        try:
            ...
            status_changed = (
                display_message != current_pod_events or pod_phase != current_pod_status
            )
        ...
        if status_changed:
            ...
            if current_status == "active":
                ...
                high_level_status = (
                    "preparing" if pod_phase != "Running" else "preparing"
                )
                logger.info(
                    f"Pod preparation status for {pod_name}: pod_phase={pod_phase}"
                )
        ...
        return {
            ...
            "is_ready": pod_phase == "Running"
            and "SSH daemon ready" in startup_message,
        }
                ...
                if type_available_gpus >= requested_gpus:
                    ...
                    if type_available_gpus == 0:
                        ...
                        try:
                            ...
                            estimated_wait_minutes = queue_position * 15
    ...
    try:
        ...
        try:
            ...
            if not current_expires_at:
                error_msg = (
                    f"No expiration time found for reservation {full_reservation_id}"
                )
                logger.error(error_msg)
        ...
        try:
            reservations_table = dynamodb.Table(RESERVATIONS_TABLE)
            update_expression = (
                "SET expires_at = :new_expires_at, last_updated = :timestamp"
            )
            expression_values = {
            ...
            update_expression += (
                " REMOVE extension_error, warnings_sent, last_warning_time"
            )
