...
def safe_create_snapshot(volume_id, user_id, snapshot_type="shutdown"):
    ...
    try:
        ...
        if ongoing_snapshots:
            ...
            logger.info(
                f"Found ongoing snapshot {latest_ongoing['SnapshotId']} for volume {volume_id}"
            )
            ...
            TagSpecifications=[
                {
                    ...
                    "Tags": [
                        {
                            "Key": "Name",
                            "Value": f"gpu-dev-{snapshot_type}-{user_id.split('@')[0]}-{timestamp}",
                        },
...
def create_pod_shutdown_snapshot(volume_id, user_id, snapshot_type="shutdown"):
    ...
    try:
        if not volume_id:
            logger.info(
                f"No persistent volume for user {user_id} - skipping {snapshot_type} snapshot"
            )
            return None

        logger.info(
            f"Creating {snapshot_type} snapshot for user {user_id}, volume {volume_id}"
        )

        # Create snapshot (or get existing one if in progress)
        snapshot_id, was_created = safe_create_snapshot(
            volume_id, user_id, snapshot_type
        )

        if was_created:
            logger.info(
                f"Started {snapshot_type} snapshot {snapshot_id} for user {user_id}"
            )
        else:
...
def cleanup_old_snapshots(user_id, keep_count=3, max_age_days=7):
    ...
    try:
        ...
        if len(snapshots) <= keep_count:
            logger.debug(
                f"User {user_id} has {len(snapshots)} snapshots, no cleanup needed"
            )
            return 0
        ...
        for i, snapshot in enumerate(snapshots):
            ...
            if snapshot_date < cutoff_date or i >= keep_count:
                try:
                    logger.info(
                        f"Deleting old snapshot {snapshot_id} from {snapshot_date}"
                    )
                    ec2_client.delete_snapshot(SnapshotId=snapshot_id)
                    deleted_count += 1
                except Exception as delete_error:
                    logger.warning(
                        f"Could not delete snapshot {snapshot_id}: {delete_error}"
                    )
